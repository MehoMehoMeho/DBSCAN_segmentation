...
def segment_dbscan(img_gray, options):
    ...
    # get mask with Otsu's thresholding
    thresh, mask = cv2.threshold(img_blur, 0, 1, cv2.THRESH_BINARY + cv2.THRESH_OTSU)
...
def main():
    ...
    cluster_list, cluster_image = segment_dbscan(img_gray, segmentation_params)

    plt.figure(1)
    show_clusters_DBSCAN(img_gray, cluster_list)
